...
# compute the benchmarks corresponding to AR6 (Table 7.SM.4). However since 
# when doing the GCAM updates we do not have future scenarios we are limited 
# in out ability to calculate some of metrics that rely on multiforcing scenarios. 
# This script is very similar to the AR6_benchamrk_fxns with the expection that 
# certain metrics are not calucated. 
#
...
# Helper function that calculates th
# Args
...
AR6B.get_tcre_fxn <- function(rslts){
  ...
  here::here(paste0("hector-", v), "input", "gcam_emissions.csv") %>% 
    read.csv(comment.char = ";") %>% 
    filter(Date %in% temp$year) %>% 
  ...
  tcre_reg <- lm(temp$value ~ net_emiss)
...
AR6B.future_warming_fxn <- function(rslts){
  ...
  temp %>%
    ...
    select(scenario, year, variable, units, version) %>% 
    mutate(value = NA) -> 
    near_temp
